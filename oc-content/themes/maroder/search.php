        ...
        $(function () {
            ...
            $("#sCity").autocomplete({
                ...
                select: function (event, ui) {
                    ...
                    log(ui.item ?
                        "<?php echo osc_esc_html(__('Selected', 'maroder')); ?>: " + ui.item.value + " aka " + ui.item.id :
                        "<?php echo osc_esc_html(__('Nothing selected, input was', 'maroder')); ?> " + this.value);
                }
            });
        });
	</script>
				...
				<div class="list-header">
					<div class="resp-wrapper">
                        ...
                        <?php if (osc_count_items() == 0) { ?>
							<p class="empty"><?php printf(__('There are no results matching "%s"', 'maroder'), osc_search_pattern()); ?></p>
                        <?php } else { ?>
							<div class="row">
								<div class="col">
									<span class="counter-search"><?php
                                        $search_number = maroder_search_number();
                                        printf(__('%1$d - %2$d of %3$d listings', 'maroder'), $search_number['from'], $search_number['to'], $search_number['of']);
                                        ?>
									</span>
								</div>
							</div>
							...
							<div class="row">
								<div class="col">
									<div class="actions">
										<a href="#" data-bclass-toggle="display-filters"
										   class="resp-toggle show-filters-btn"><?php _e('Show filters', 'maroder'); ?></a>
										<!--     START sort by       -->
										<span class="see_by">
              <span><?php _e('Sort by', 'maroder'); ?>:</span>
              <?php
              ...
              <ul>
                  ...
                  foreach ($orders as $label => $params) {
                      ...
                      <?php if (osc_search_order() == $params['sOrder'] && osc_search_order_type() == $orderType) { ?>
						  <li><a class="current"
								 href="<?php echo osc_esc_html(osc_update_search_url($params)); ?>"><?php echo $label; ?></a></li>
                      <?php } else { ?>
                  ...
                  <?php } ?>
                   ...
                   <a href="<?php echo osc_esc_html(osc_update_search_url(array('sShowAs' => 'list'))); ?>"
					  class="list-button" data-class-toggle="listing-grid"
                   ...
                   <a href="<?php echo osc_esc_html(osc_update_search_url(array('sShowAs' => 'gallery'))); ?>"
					  class="grid-button" data-class-toggle="listing-grid"
                ...
                if (osc_count_premiums() > 0) {
                    echo '<h5>' . __('Premium listings', 'maroder') . '</h5>';
                ...
                ?>
                <?php if (osc_count_items() > 0) {
                    echo '<h5>' . __('Listings', 'maroder') . '</h5>';
                    ...
                    if (osc_rewrite_enabled()) {
                        ...
                        if (count($footerLinks) > 0) {
							...
							<div id="related-searches">
								<h5><?php _e('Other searches that may interest you', 'maroder'); ?></h5>
